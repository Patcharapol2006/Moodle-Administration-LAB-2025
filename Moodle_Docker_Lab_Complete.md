...
**บันทึกผลการทดลอง 3:**
<img width="1077" height="158" alt="Screenshot 2026-02-06 094838" src="https://github.com/user-attachments/assets/e7b5c789-41c4-43c8-95e7-9a91f67b25b5" />
...
```
**บันทึกผลการทดลอง 4:**
<img width="1076" height="191" alt="Screenshot 2026-02-06 100239" src="https://github.com/user-attachments/assets/c7908619-5393-4a3a-993e-df84ff048b83" />
...
<img width="1919" height="870" alt="Screenshot 2026-02-19 032357" src="https://github.com/user-attachments/assets/bf5962a9-88b1-4421-b3d6-455e5bab3fae" />
...
<img width="1912" height="885" alt="Screenshot 2026-02-19 032536" src="https://github.com/user-attachments/assets/faedb330-9992-4b36-bf82-416766b8208a" />
...
<img width="1911" height="882" alt="Screenshot 2026-02-19 033524" src="https://github.com/user-attachments/assets/3c9d1462-2e47-4883-a014-0e6aeefa5961" />
...
<img width="1887" height="856" alt="Screenshot 2026-02-19 033740" src="https://github.com/user-attachments/assets/b4576d0c-5850-41e4-8af4-5340df21b1dc" />
...
<img width="1536" height="480" alt="Screenshot 2026-02-19 034237" src="https://github.com/user-attachments/assets/1c829ca5-a2cc-4d0b-b475-2e3d162a26d3" />
...
<img width="1503" height="675" alt="image" src="https://github.com/user-attachments/assets/0028c36d-7e82-47ab-9833-d81aca036cf1" />
...
คำตอบ:
```
Docker Image: เป็นไฟล์ต้นแบบที่แก้ไขไม่ได้ (Read-only) บรรจุซอฟต์แวร์และสภาพแวดล้อมที่จำเป็น เปรียบเหมือน "พิมพ์เขียว" หรือ "แผ่นโปรแกรมติดตั้ง"

Docker Container: เป็นร่างที่ทำงานจริง (Instance) ที่สร้างมาจาก Image เปรียบเหมือน "บ้านที่สร้างเสร็จแล้ว" หรือ "โปรแกรมที่กำลังรันอยู่"

ตัวอย่าง: Image คือไฟล์ติดตั้ง Windows ส่วน Container คือคอมพิวเตอร์ที่กำลังรัน Windows เครื่องนั้นๆ อยู่
```
...
Moodle Container (moodle_app): ทำหน้าที่เป็น Web Server ประมวลผลภาษา PHP และให้บริการหน้าเว็บไซต์ Moodle

Database Container (db): ทำหน้าที่เป็นระบบจัดการฐานข้อมูล (เช่น MariaDB/MySQL) เพื่อเก็บข้อมูลผู้ใช้ รายวิชา และคะแนน
...
คำตอบ:
```
ประเภท: เป็นการใช้ Named Volumes (เช่น moodle_data, db_data)

ข้อดี: ข้อมูลไม่หายเมื่อลบ Container (Persistence), Docker จัดการตำแหน่งที่เก็บไฟล์ให้โดยอัตโนมัติ

ข้อเสีย: เข้าถึงไฟล์โดยตรงจาก Host OS ได้ยากกว่าแบบ Bind Mount (ต้องเข้าไปใน Directory เฉพาะของ Docker)
```
...
คำตอบ:
```
หน้าที่: สร้างเครือข่ายเสมือน (Virtual Network) เพื่อให้ Container ที่อยู่ในกลุ่มเดียวกันรู้จักกัน

การสื่อสาร: สื่อสารกันผ่าน Service Name (เช่น Moodle เชื่อมต่อหา DB โดยใช้ชื่อ Host ว่า db) แทนการใช้ IP Address
...
คำตอบ:
```
ความสำคัญ: ใช้กำหนดลำดับการเริ่มทำงาน (Startup Order) โดยสั่งให้ Container หนึ่ง (เช่น db) เริ่มทำงานก่อนอีกตัวหนึ่ง (moodle_app) เพื่อป้องกัน Error จากการที่แอปพลิเคชันทำงานก่อนที่ฐานข้อมูลจะพร้อม
```
...
คำตอบ:
```
การแก้ไข: แก้ไขที่หัวข้อ ports: ใน Service ของ Moodle จากเดิม 80:80 เป็น 9000:80 (Host_Port:Container_Port)
...
คำตอบ:
```
หมายความว่า: บอกให้ Moodle ไปเชื่อมต่อฐานข้อมูลที่เครื่องชื่อ db (ซึ่งเป็นชื่อ Service ใน Docker Compose)

เหตุผลที่ไม่ใช้ localhost: เพราะในระบบ Container คำว่า localhost จะหมายถึงตัว Container เอง ถ้าใช้ใน moodle_app มันจะพยายามหา DB ในตัวมันเองซึ่งไม่มี
```
...
คำตอบ:
```
ข้อดี: ติดตั้งง่ายและเร็ว (จบในคำสั่งเดียว), สภาพแวดล้อมเสถียร (ย้ายเครื่องไปไหนก็รันได้เหมือนเดิม), ลบและติดตั้งใหม่ได้ง่ายไม่รกเครื่อง

ข้อเสีย: ประสิทธิภาพอาจลดลงเล็กน้อยจากการผ่าน Layer ของ Docker, ต้องมีความรู้เรื่องคำสั่ง Docker เพิ่มเติม
```
...
วิธีตรวจสอบ: ใช้คำสั่ง docker-compose logs moodle_app เพื่อดู Error message หรือ docker ps เพื่อดูว่า Container db รันอยู่หรือไม่

วิธีแก้ไข: ตรวจสอบว่า MOODLE_DB_PASSWORD ตรงกันทั้งสองเครื่องหรือไม่ และเช็คว่าทั้งคู่ใช้ networks เดียวกันหรือไม่
...
คำตอบ: Container จะถูกลบออก และ ข้อมูลทั้งหมดที่อยู่ใน Volume จะถูกลบถาวร (พารามิเตอร์ -v คือการสั่งลบ Volumes ทิ้งด้วย)
```
